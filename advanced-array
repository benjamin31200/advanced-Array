...
const instructors = [
  ...
  const onlyJS = [];
  const msgJS = (a) => 
    alert('Hi ' + a + ', we inform you that this weekend you will be doing the support class');
  
  const msgPython = () => 
    alert('And you need to prepare a Python workshop');
  
  function available(JS, week, instructors) {
    if (JS.some(x => x === 'Javascript') === true
       && (week === 'weekend' || 'all')) {
      onlyJS.push(instructors);  
    }
    return onlyJS;
  }
  for (let i = 0; i < instructors.length; i++) {
    available(instructors[i].specialities, instructors[i].availability, instructors[i]);
  }
  
  for (const instructor of onlyJS) {
     msgJS(instructor.name);
    if(instructor.specialities.some(x => x === 'Python')) {
      msgPython();
    }
    }
